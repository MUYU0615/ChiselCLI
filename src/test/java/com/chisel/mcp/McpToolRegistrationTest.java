...
class McpToolRegistrationTest {
    ...
    private static void withAuditDir(Path tempDir, ThrowingRunnable body) throws Exception {
        String previous = System.getProperty("chisel.audit.dir");
        System.setProperty("chisel.audit.dir", tempDir.resolve("audit").toString());
        try {
            body.run();
        } finally {
            if (previous == null) {
                System.clearProperty("chisel.audit.dir");
            } else {
                System.setProperty("chisel.audit.dir", previous);
            }
        }
    }
