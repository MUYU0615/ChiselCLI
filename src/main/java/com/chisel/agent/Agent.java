package com.chisel.agent;

import com.chisel.llm.LlmClient;
import com.chisel.llm.LlmTraceLogger;
import com.chisel.context.ContextProfile;
import com.chisel.context.TokenUsageFormatter;
import com.chisel.lsp.LspDiagnosticReport;
import com.chisel.memory.ConversationHistoryCompactor;
import com.chisel.memory.ExplicitMemoryHints;
import com.chisel.memory.MemoryManager;
import com.chisel.prompt.PromptAssembler;
import com.chisel.prompt.PromptContext;
import com.chisel.prompt.PromptMode;
import com.chisel.prompt.ProjectMemoryLoader;
import com.chisel.render.PlainRenderer;
import com.chisel.render.Renderer;
import com.chisel.render.StatusInfo;
import com.chisel.runtime.CancellationContext;
import com.chisel.skill.SkillContextBuffer;
import com.chisel.skill.SkillIndexFormatter;
import com.chisel.skill.SkillRegistry;
import com.chisel.util.AnsiStyle;
import com.chisel.tool.ToolRegistry;
import com.chisel.tool.ToolRegistry.ToolExecutionResult;
import com.chisel.tool.ToolRegistry.ToolInvocation;
import com.chisel.util.TerminalMarkdownRenderer;
import com.chisel.image.ImageReferenceParser;
...
public class Agent {
    ...
    public String run(String userInput) {
        ...
        while (true) {
            ...
            // 这是与 Memory 短期记忆压缩并行的另一道压缩——后者只压 shortTermMemory，
    ...
    public String getContextStatus() {
        com.chisel.context.ContextProfile profile = memoryManager.getContextProfile();
        ...
        for (LlmClient.Message msg : conversationHistory) {
            int t = com.chisel.memory.TokenBudget.estimateMessagesTokens(java.util.List.of(msg));
            switch (msg.role()) {
    ...
    private int estimateToolsSchemaTokens() {
        try {
            return com.chisel.memory.MemoryEntry.estimateTokens(
    ...
    private long estimateCurrentContextTokens() {
        long messageTokens = com.chisel.memory.TokenBudget.estimateMessagesTokens(conversationHistory);
    ...
    private void logRequestContext(String scope, List<LlmClient.Tool> tools) {
        ...
        for (int messageIndex = 0; messageIndex < conversationHistory.size(); messageIndex++) {
            ...
            messages++;
            int tokens = com.chisel.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
        ...
        if (tools != null && !tools.isEmpty()) {
            try {
                toolsSchemaTokens = com.chisel.memory.MemoryEntry.estimateTokens(
