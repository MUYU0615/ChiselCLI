 ...
 * 1. 系统属性：{@code chisel.react.token.budget} / {@code chisel.react.stagnation.window} /
 *    {@code chisel.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -Dchisel.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -Dchisel.react.token.budget=N 仍可启用硬预算，覆盖默认。
        return new AgentBudget(
                readIntProperty("chisel.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("chisel.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("chisel.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
