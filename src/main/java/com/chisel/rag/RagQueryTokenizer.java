...
import com.chisel.util.JiebaSegmenterFactory;
...
final class RagQueryTokenizer {
    ...
    private static boolean isUsefulToken(String token) {
        ...
        // 高频停用词：问句虚词 + 代码场景高频泛词（这些词在大量代码块注释/内容里出现，
        // 若作为关键词做 LIKE 会大面积误命中，污染关键词路径的加权）
        boolean stopword = switch (lower) {
            case "怎么", "如何", "什么", "哪些", "一下", "实现", "的是", "一个", "可以", "这里", "那里",
                 "工具", "调用", "代码", "逻辑", "系统", "项目", "功能", "处理", "使用", "需要", "进行",
                 "相关", "以及", "通过", "就是", "这个", "那个", "因为", "所以", "但是", "如果",
                 "没有", "不是", "还是", "哪个", "哪里", "为什么", "能否",
                 "有没有", "是不是", "应该", "查看", "找到", "看下", "帮我", "请" -> true;
            default -> false;
        };
