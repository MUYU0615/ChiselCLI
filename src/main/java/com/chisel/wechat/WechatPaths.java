...
public final class WechatPaths {
    ...
    public static Path root() {
        String configured = System.getProperty("chisel.wechat.dir");
        if (configured == null || configured.isBlank()) {
            configured = System.getenv("CHISEL_WECHAT_DIR");
        }
        if (configured == null || configured.isBlank()) {
            configured = Path.of(System.getProperty("user.home"), ".chisel", "wechat").toString();
        }
    ...
    public static Path pidFile() {
        return root().resolve("chisel-wechat.pid");
    }
}
