...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("chisel.snapshot.enabled", "CHISEL_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("chisel.snapshot.dir", "CHISEL_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".chisel", "snapshots").toString()));
        int max = readInt("chisel.snapshot.max", "CHISEL_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("chisel.snapshot.excludes", "CHISEL_SNAPSHOT_EXCLUDES", ""));
