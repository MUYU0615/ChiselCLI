...
import com.chisel.render.inline.TerminalCapabilities;
 ...
 * <p>选型规则：
 * <ul>
 *   <li>{@code -Dchisel.renderer} > {@code CHISEL_RENDERER} 环境变量 > 默认 inline</li>
 ...
 *   <li>兼容：{@code CHISEL_TUI=true} → 等价 {@code lanterna}，打 deprecation 提示</li>
 * </ul>
 *
...
public final class RendererFactory {
    ...
    public static Mode resolveMode() {
        ...
        // 兼容旧 CHISEL_TUI=true → lanterna
        String legacyTui = System.getenv("CHISEL_TUI");
        if (legacyTui != null && Boolean.parseBoolean(legacyTui.trim())) {
            System.err.println("⚠️ CHISEL_TUI=true 已废弃，请改用 CHISEL_RENDERER=lanterna");
    ...
    private static Mode parse(String raw) {
        return switch (raw.trim().toLowerCase()) {
            ...
            default -> {
                System.err.println("⚠️ 未识别的 CHISEL_RENDERER='" + raw + "'，回退到 inline");
                yield Mode.INLINE;
            }
        };
    }
