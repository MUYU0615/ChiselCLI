...
- `AuditLog` 操作审计链：危险工具调用按天写 JSONL 到 `~/.chisel/audit/`，含 `outcome (allow|deny|error)` 与 `approver (hitl|policy|none)`
...
- ChiselCLI 的安全模型是 **HITL + 路径校验 + 命令快速拒绝 + 审计**，不是隔离
...
## 多模型适配 + 运行时切换（GLM / DeepSeek / StepFun / Kimi）✅
...
- 配置持久化：`~/.chisel/config.json` 存储默认模型，支持 `.env` 回退读取 API Key
...
**功能迭代**：
- `web_search` 工具升级：在 SerpAPI 最小落地的基础上，把搜索结果结构化、字段稳定化
...
## MCP 协议核心（stdio + Streamable HTTP，默认开启） ✅

**已完成**

**目标**：把 ChiselCLI 接入 MCP 生态。stdio 子进程 server 与 Streamable HTTP 远程 server 都能用，工具自动注册到 ToolRegistry，与 HITL / AuditLog 协同。
...
- 配置文件：`~/.chisel/mcp.json`（用户级）+ `.chisel/mcp.json`（项目级，可入 git），格式与 Claude Code `claude_desktop_config.json` 兼容
- 启动时 eager 并行启动所有 server（复用并行调度）
...
- resources 自动注入 system prompt（长上下文模式已接入 URI / 描述索引）
...
## 长上下文工程（适配 200k–1M 模型 + prompt caching） ✅

**目标**：适配 GLM-5.1（200k）/ DeepSeek V4（1M）/ StepFun（256k）/ Kimi K2.6（256k）/ Claude Sonnet 4.6（1M）等长上下文模型。 Memory 是基于"短上下文兜底"假设设计的，长窗口下要切换策略。
...
- **MCP resources 自动注入**（与联动）：长模式下，把所有 server 已知 resources 的 URI + 描述（不含 body）作为索引注入 system prompt；ReAct / Plan / Team 都接入
...
- **默认 enabled**：`~/.chisel/mcp.json` 不存在时启动自动创建模板，含 chrome-devtools 条目
- `image` content 处理走**路线 B**：fallback 文案引导 LLM 优先用 `take_snapshot`（DOM 文本快照）而非 `take_screenshot`；不做真 图片复制粘贴输入（拆到）
...
- `McpClient.initialize` 超时 30s → 60s（chrome-devtools 首次启动需 npx 拉包 + Chrome 冷启 ≈ 20s+），可被 `chisel.mcp.initialize.timeout.seconds` 覆盖
...
**不做（明确边界）**：
- 真 图片复制粘贴输入（拆到「图片复制粘贴输入」）
- CDP 会话复用 / 登录态识别（）
- Playwright / Firefox / WebKit 跨浏览器
- 浏览器执行隔离（默认 `--isolated=true` 临时 user-data-dir，通过 `--autoConnect` 或旧式 `--browser-url` 复用已开 Chrome）
...
**前置依赖**：Chrome DevTools MCP 已能驱动浏览器
...
## Skill 系统 + web-access Skill ✅

**已完成**

**前置依赖**： web 工具、 Chrome DevTools MCP、 CDP 会话复用全部就绪

**目标**：做出 ChiselCLI 自己的 Skill 加载机制，把零散的工具与决策指引打包成可复用单元，并以 web-access 作为首个落地 Skill

**功能迭代**（详细开发任务见 `docs/phase-15-skill-system.md`）：
- Skill 加载机制：三层目录扫描（jar 内置 / 用户级 `~/.chisel/skills/` / 项目级 `<project>/.chisel/skills/`），按 name 整体覆盖，frontmatter 走手写 YAML 子集解析（不引 SnakeYAML）
...
- 启动期 `SkillBuiltinExtractor` 把 jar 内置 skill 解压到 `~/.chisel/skills-cache/`，按 `.version` 文件控制重建
- CLI 命令：`/skill` / `/skill list` / `/skill show <name>` / `/skill on <name>` / `/skill off <name>` / `/skill reload`
- Jina Reader 集成：**只**在 web-access SKILL.md 写入「web_fetch 失败可让 execute_command 调 r.jina.ai」的提示，**不**改 `web_fetch` 工具内部链路（保持纯本地约定）
...
- 切换：`CHISEL_RENDERER=inline|lanterna|plain`，旧 `CHISEL_TUI=true` 兼容映射到 lanterna
...
**设计参考**：DeepSeek TUI `crates/tui/src/lsp/`——`LspManager`（惰性 transport pool）+ `lsp_hooks.rs`（post-edit 挂钩）+ `diagnostics.rs`（诊断类型与渲染）。ChiselCLI 的 Java 生态可以用 Eclipse JDT LS（`org.eclipse.jdt.ls`）或直接复用已有的 `CodeAnalyzer` 做轻量版。
...
**功能迭代**：
- `SideGitManager`：在 `~/.chisel/snapshots/<project_hash>/<worktree_hash>/.git` 维护独立 side-git 仓库，通过 JGit 纯 Java 实现，与用户的工作区 `.git` 完全隔离
...
**当前状态**：MVP 已落地。ReAct、Plan task executor、Multi-Agent 三角色、Planner 已接入 `PromptAssembler`，内置资源位于 `src/main/resources/prompts/`，覆盖路径支持 `~/.chisel/prompts/...` 与 `.chisel/prompts/...`。
...
- 用户级覆盖：`~/.chisel/prompts/base.md` 可整体替换内置 base.md；`~/.chisel/prompts/modes/agent.md` 可覆盖特定模式；项目级 `.chisel/prompts/...` 优先级更高
...
## 异步后台任务 + Runtime API（异步 & 无头场景） ✅ MVP

**前置依赖**： Chrome DevTools MCP 已能产出截图等 image content；长上下文工程已就绪。

**目标**：用户可以在 TUI 里提交后台任务（如"重构整个模块"），关掉终端走人，回来查看结果。同时暴露 HTTP/SSE Runtime API，让 ChiselCLI 可以嵌入 CI/CD、IDE 插件、Web 面板。
...
**Runtime API**：
- `RuntimeApiServer`：嵌入式 HTTP/SSE 服务端（`chisel serve --http --port 8080`），基于已有的 OkHttp / Javalin 或 Spring Boot 内嵌
...
**当前 MVP 已落地**：
- `DurableTaskManager`：SQLite 后台任务队列，默认 `~/.chisel/tasks/tasks.db`
- `/task`、`/task add`、`/task cancel`、`/task log` CLI 闭环
- 进程启动时将残留 `running` 任务恢复为 `enqueued`
- Worker Pool 默认 2，可用 `CHISEL_TASK_WORKERS` / `-Dchisel.task.workers` 覆盖
- `RuntimeApiServer`：基于 JDK `HttpServer`，仅监听 `127.0.0.1`
- `RuntimeThreadStore`：SQLite 保存 thread 与 event 时间线
- Runtime API 强制 `CHISEL_RUNTIME_API_KEY` / `-Dchisel.runtime.api.key`
...
**前置依赖**： Chrome DevTools MCP 已能产出截图等 image content；长上下文工程已就绪；第 17–20 期安全网与架构已就绪。

**目标**：让 ChiselCLI 真正"看见"图片——浏览器截图、用户粘贴的图片、文档中的图表，都能直接喂给 LLM 让它理解，而不是 fallback 成"[此工具返回了 image]"占位。此期排在安全网（LSP + 快照）和架构重构（Prompt + Task）之后，确保模型生态成熟时再做。
...
- 的 `take_screenshot` image fallback 升级为图片附件回灌；输入层不按模型名拦截图片
...
**不做**：
- 视频 / 音频输入（再独立期）
- 图像生成（ChiselCLI 是 Agent 不是绘图工具）
- TUI sixel 协议显示截图（依赖 TUI 是否实现，留作扩展）
...
```
基础ReAct ──► 规划执行 ──► 记忆上下文 ──► RAG检索 ──► 多Agent协作 ──► 人机协同 ──► 异步并行 ──► 多模型

联网能力 ──► MCP核心 ──► MCP高级 ──► 长上下文 ──► Chrome DevTools ──► CDP会话复用 ──► Skill系统 ──► TUI ──► LSP诊断
