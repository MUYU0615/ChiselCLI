...
当前基准（2025-08 实测，本地弱向量模型）：RAG 命中率 8/8 = 100%，grep 命中率 3/8 = 37.5%——
语义检索在模糊查询上命中率约为 grep 的 2.7 倍；RAG 单独命中的用例（JSON-RPC 配对 / 快照回滚 / 微信策略 / 死循环检测 / 审批逻辑 / 限流 / token 刷新）
均为 grep 完全失败、只能靠语义或符号命中定位的场景。

检索加权设计（从 50% 提到 100% 的四次修复）：
- 符号精确命中（类名/方法名含关键词）权重最高（+1.0）；关键词在方法体/注释内（如 stagnant 字段）也有显著权重（+0.8），
  但泛中文词（工具/调用/代码等）content 命中减半（+0.4），避免大面积误命中把真正符号挤到同分并列。
- 索引排除 src/test：测试代码会污染检索结果、挤占生产代码排名（chunk 从 3477 → 2303）。
