...
1. `~/.chisel/config.json` 中对应 provider 的 `apiKey`
...
| 数据 | 默认路径 | 覆盖方式 |
|------|----------|----------|
| 长期记忆 | `~/.chisel/memory/long_term_memory.json` | `-Dchisel.memory.dir` |
| 项目级记忆 | `PAI.md` / `.chisel/PAI.md` / `PAI.local.md` | 用户级稳定偏好：`~/.chisel/PAI.md` |
| RAG 索引 | `~/.chisel/rag/codebase.db` | `-Dchisel.rag.dir` |
| 审计日志 | `~/.chisel/audit/audit-YYYY-MM-DD.jsonl` | `CHISEL_AUDIT_DIR` / `-Dchisel.audit.dir` |
| Side-Git 快照 | `~/.chisel/snapshots/<project_hash>/<worktree_hash>/.git` | `CHISEL_SNAPSHOT_DIR` / `-Dchisel.snapshot.dir` |
| 后台任务 | `~/.chisel/tasks/tasks.db` | — |

### Snapshot Config

系统属性 > 环境变量 > 默认值：`chisel.snapshot.enabled`(true) / `chisel.snapshot.max`(50) / `chisel.snapshot.excludes`(.git,.chisel/snapshots,target,node_modules,dist,.idea,*.class,*.jar) / `chisel.snapshot.dir`(~/.chisel/snapshots)
...
系统属性 > 环境变量/.env > 默认值：`CHISEL_LOG_DIR`(~/.chisel/logs) / `CHISEL_LOG_LEVEL`(INFO) / `CHISEL_LOG_MAX_HISTORY`(7) / `CHISEL_LOG_MAX_FILE_SIZE`(10MB) / `CHISEL_LOG_TOTAL_SIZE_CAP`(100MB)

### ReAct/SubAgent Budget Config

系统属性 > 默认值：`chisel.react.token.budget`(Integer.MAX_VALUE) / `chisel.react.stagnation.window`(3) / `chisel.react.hard.max.iterations`(50)
...
系统属性 > 默认值：`chisel.llm.connect.timeout.seconds`(60) / `chisel.llm.read.timeout.seconds`(300) / `chisel.llm.write.timeout.seconds`(60) / `chisel.llm.call.timeout.seconds`(600)
...
1. 用户级：`~/.chisel/mcp.json`
2. 项目级：`.chisel/mcp.json`
...
- 加载顺序：`~/.chisel/PAI.md` → `PAI.md` → `.chisel/PAI.md` → `PAI.local.md` → `.chisel/PAI.local.md`
...
- shared 模式 close_page 只允许关闭 ChiselCLI 创建的 tab

### Skill System

- 三层加载：jar 内置 < 用户级 ~/.chisel/skills/ < 项目级 .chisel/skills/
...
- 环境变量：`CHISEL_RENDERER=inline|lanterna|plain`
- `CHISEL_TUI=true`(旧) → lanterna + deprecation 提示
- `CHISEL_NO_STATUSBAR=true`：禁用底部状态栏
...
- `/export` 导出当前 ReAct conversationHistory 为 Markdown 到 `~/.chisel/exports/session-*.md`；包含完整 system prompt，便于检查 LLM 实际接收前的指令，命令不接受路径参数。
...
- `CHISEL_LSP_ENABLED=false` 关闭

### Git Side-History Snapshot (Phase 18)

- side-git 在 ~/.chisel/snapshots/ 维护独立仓库（JGit，不依赖系统 git）
...
- 覆盖优先级：jar 内置 < 用户级 ~/.chisel/prompts/ < 项目级 .chisel/prompts/
...
- XfyunMaaSClient：Qwen3.6-35B-A3B，默认 https://maas-api.cn-huabei-1.xf-yun.com/v2，OpenAI-compatible 讯飞星辰 MaaS；可用 `/config provider xfyun ...` 写入配置后 `/model xfyun` 切换。`model` 必须使用 MaaS 服务管控页展示的 modelId；微调模型可配置 `--lora-id <resourceId>`，作为 HTTP header `lora_id` 发出；该 provider 不发送 ChiselCLI 内置 tools。
...
# CHISEL_SNAPSHOT_ENABLED=true
# CHISEL_SNAPSHOT_MAX=50
# CHISEL_SNAPSHOT_EXCLUDES=.git,.chisel/snapshots,target,node_modules,dist,.idea,*.class,*.jar
# CHISEL_SNAPSHOT_DIR=/Users/yourname/.chisel/snapshots
# CHISEL_TUI=true
# NO_TUI=true
```
