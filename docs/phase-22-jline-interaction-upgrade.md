...
本阶段目标：把 ChiselCLI 从“使用 JLine 的 CLI”升级为“以 JLine 4 为核心 UI runtime 的 Agent 终端”，交互体验对齐 Claude Code / Qoder CLI 一类产品。
...
- 开屏改为凿子（Chisel）主题彩色 logo + Qoder 风格首屏；MCP/Skill 初始化完成后一次性展示模型、MCP、Skill、ReAct 状态和三条 getting-started tips。
...
- 增加 `ChiselHighlighter`：高亮 `/command`、`@path`、`@image:`、`@clipboard`。
- 对危险命令关键词做视觉提示。
- 对未闭合括号/引号做轻量提示。
...
- 已扩展 `ChiselCompleter` 的上下文补全入口，避免多个输入态各自维护补全逻辑。
...
- 已接入 `ChiselHistory`，基于 JLine `DefaultHistory` 做 ChiselCLI 过滤策略。
- 默认配置 `~/.chisel/history/input.history` 持久化，可用 `chisel.history.file` / `CHISEL_HISTORY_FILE` 覆盖；如果配置值指向目录，则使用该目录下的 `input.history`。
- 已设置 history size / file size，默认 `2000` / `10000`，可用 `chisel.history.size`、`chisel.history.fileSize` 或对应环境变量覆盖。
- 已忽略空白、重复、超长输入。
...
- 暂不使用 `jline-shell` 替换 ChiselCLI 现有命令解析。
...
- HITL 审批期间不吞输入。
- `java -jar target/chisel-1.0-SNAPSHOT.jar` 真机验证。
